...
DISCRIMINATE_BKG = False
SIGNAL_DEC_IDS     = ["40114060"]
BACKGROUND_DEC_IDS = ["30011001"]  # default MUON, overridden by CLI
SIGNAL_DATA_TYPE   = "signal"
OUTPUT_NAME        = f"pileup_{BKG_TYPE}_CODEX_GNN"

BATCH_SIZE    = 8      # grafos ~18K nodos, ~350K aristas cada uno
...
VAL_WORKERS   = 4
MP_CONTEXT    = 'fork'  # scratch NVMe: no Lustre deadlocks, fork is fast
MAX_VAL_PAIRS = 7       # 7 validation chunk pairs
...
def train():
    ...
    # Use 'fork' context (safe on scratch NVMe, no Lustre deadlocks).
    # persistent_workers=True keeps workers alive between epochs,
    # preserving the LRU cache so chunks are only read ONCE.
    ...
    os.makedirs(f"models/pileup_{BKG_TYPE}", exist_ok=True)
    checkpoint = ModelCheckpoint(
        dirpath=f"models/pileup_{BKG_TYPE}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train CODEX-b GNN on pileup-mixed data."
    )
    parser.add_argument(
        "--bkg-type", choices=["MUON", "KL0"], default="MUON",
        help="Background type to discriminate against (default: MUON)."
    )
    args = parser.parse_args()

    # Recompute globals based on --bkg-type
    ...
    print(f"🚀 Training pileup model: signal={SIGNAL_DEC_IDS}, "
          f"background={BACKGROUND_DEC_IDS}")
